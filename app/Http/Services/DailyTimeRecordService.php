<?php
    ...
    class DailyTimeRecordService {
        ...
        public function get_programs($request){ 
            if($request->query('prog_id') === null){
                $programs = Program::select('programName', 'id')
                ->distinct()->orderBy('programName', 'ASC')
                ->paginate(isset($request->limit) ? $request->limit : 30);
            }else{
        ...
        public function get_program_names() : array{
            $program_names = [];
            Program::select('programName', 'id')
                            ->distinct()->orderBy('programName', 'ASC')
                            ->chunk(100, function($programs) use (&$program_names) {
            ...
            foreach ($programs as $program_key => $program) {
                ...
                if(count($campaigns) > 0){
                    foreach ($campaigns as $campaign_key => $campaign) {
                        ...
                        foreach ($distinct_dates as $date) {
                            $final_data[$program_key]['children'][$campaign_key]['training_hours_' . $date] = 0;
                        }
        ...
        public function get_campaign_data($final_data = [], $distinct_dates){
            ...
            Campaign::whereIn('account_id', $program_ids)->chunk(50, function($campaigns) use (&$final_data, $distinct_dates){
                for ($i=0; $i < count($final_data); $i++) { 
                    ...
                    foreach ($campaigns as $campaign_key => $campaign) {
                        if($campaign->account_id == $prog_id){
                            ...
                            foreach ($distinct_dates as $date) {
                                $final_data[$i]['children'][$campaign_key]['training_hours_' . $date] = 0;
                            }
                ...
                foreach($final_data[$i]['children'] as $campaign_key  => $campaign){
                    foreach($campaign['children'] as $agent_key => $agent){
                        foreach($date_data as $date){
                            $final_data[$i]['children'][$campaign_key]['ot_hours_' . $date]       += number_format($agent['ot_hours_' . $date], 2);
                            $final_data[$i]['children'][$campaign_key]['total_hours_' . $date]    += number_format($agent['total_hours_' . $date], 2);
                            $final_data[$i]['children'][$campaign_key]['billable_hours_' . $date] += number_format($agent['billable_hours_' . $date], 2);
                            $final_data[$i]['children'][$campaign_key]['training_hours_' . $date] += number_format($agent['training_hours_' . $date], 2);
                        }
                    }
                }
            }
        ...
        public function calculate_agent_total_row($final_data = [], $dates){
                ...
                foreach($final_data[$i]['children'] as $campaign_key  => $campaign){
                    foreach($campaign['children'] as $agent_key => $agent){
                        foreach($dates as $date){
                            ...
                            $final_data[$i]['children'][$campaign_key]['children'][$agent_key]['training_hours' ] += $agent['training_hours_' . $date];
                        }
                    }
                }
            }
            ...
            /**
             * total hours !> 40
	         * REDUCE IF GREATER THAN 40
             */
            for ($i = 0; $i < count($final_data); $i++) { 
                foreach($final_data[$i]['children'] as $campaign_key  => $campaign){
                    $campaign_data = Campaign::where('id', $campaign['campaign_id'])->first();
                    foreach($campaign['children'] as $agent_key => $agent){
                        foreach($dates as $date){
                            $agent_total_hours = $final_data[$i]['children'][$campaign_key]['children'][$agent_key]['total_hours' ];
                            $agent_training_hours = $final_data[$i]['children'][$campaign_key]['children'][$agent_key]['training_hours' ];
                            if($agent_total_hours > 40){
                                $final_data[$i]['children'][$campaign_key]['children'][$agent_key]['total_hours' ] = 40;
                            }
                            $final_data[$i]['children'][$campaign_key]['children'][$agent_key]['training_amount' ] = $agent_training_hours * $campaign_data->in_training_rate;
                        }
                    }
                }
            }
        ...
        public function calculate_campaign_total_row($final_data = [], $dates, $date_array){
            ...
            for ($i = 0; $i < count($final_data); $i++) { 
                foreach($final_data[$i]['children'] as $campaign_key  => $campaign){
                    ...
                    foreach($dates as $date){
                        ...
                        $final_data[$i]['children'][$campaign_key]['training_hours' ] += $campaign['training_hours_' . $date];
                    }
                }
            }
            ...
            // billable_amount
            // premium_amount
            // CAMPAIGN = TOTAL HOURS x BASE RATE / hourly rate = billable amount
            // BY CAMPAGIGN = TOTAL OT HOURS x PREMIUM = PREMIUM amount
            
            for ($i = 0; $i < count($final_data); $i++) { 
                foreach($final_data[$i]['children'] as $campaign_key  => $campaign_a){
                    $campaign = Campaign::where('id', $campaign_a['campaign_id'])->first();
                    $final_data[$i]['children'][$campaign_key]['billable_amount'] = $campaign_a['total_hours'] * (float)$campaign->hrly_rate;
                    $final_data[$i]['children'][$campaign_key]['premium_amount']  = $campaign_a['total_ot_hours'] * (float)$campaign->premium;
                }
            }
        ...
        public function calculate_total_amount($final_data = []) : array{

            //agent part calculation
            // 'key'                  => $final_data[$i]['key'] . '-' . $agent->campaignID . '-' . $agent->id,
            // 'emp_id'               => $agent->id,
            // 'description'          => 'Agents / Employee',
            // 'name'                 => $agent->fName . ' ' . $agent->lName . ' ' . $agent->mName,
            ...
            for ($i=0; $i < count($final_data); $i++) { 
                foreach ($final_data[$i]['children'] as $campaign_key => $campaign) {
                    foreach ($campaign['children'] as $agent_key => $agent) {
                        $total_amount = $agent['billable_amount'] + $agent['premium_amount'] + $agent['training_amount'];
                        $final_data[$i]['children'][$campaign_key]['children'][$agent_key]['total_amount'] = $total_amount;
                    }
                }
            }
            ...
            for ($i=0; $i < count($final_data); $i++) { 
                foreach ($final_data[$i]['children'] as $campaign_key => $campaign) {
                    $total_amount = $campaign['billable_amount'] + $campaign['premium_amount'] + $campaign['training_amount'];
                    $final_data[$i]['children'][$campaign_key]['total_amount'] = $total_amount;
                }
            }

            for ($i=0; $i < count($final_data); $i++) { 
                $total_amount = $final_data[$i]['billable_amount'] + $final_data[$i]['premium_amount'] + $final_data[$i]['training_amount'];
                $final_data[$i]['total_amount'] = $total_amount;
            }
